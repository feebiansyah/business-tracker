...
import { getAdsterraCampaignStatusesAction, setAdsterraAutoScheduleEnabledAction, setAdsterraCampaignActiveAction, syncAdsterraDailyAction } from "@/app/shopee/[id]/adsterra-roi/actions";
...
type Campaign = { id: number; campaignId: string; label: string | null; sourceTag: string; autoScheduleEnabled: boolean };
...
export function AdsterraCampaignReportList({ shopeeAccountId, campaigns, connected }: { shopeeAccountId: number; campaigns: Campaign[]; connected: boolean }) {
  ...
  const [autoScheduleByConfig, setAutoScheduleByConfig] = useState<Record<number, boolean>>(() => Object.fromEntries(campaigns.map((campaign) => [campaign.id, campaign.autoScheduleEnabled])));
  const [autoScheduleBusyIds, setAutoScheduleBusyIds] = useState<Set<number>>(new Set());
  ...
  const autoScheduleLocks = useRef(new Set<number>());
  ...
  async function toggleAutoSchedule(campaign: Campaign, enabled: boolean) {
    if (autoScheduleLocks.current.has(campaign.id)) return;
    autoScheduleLocks.current.add(campaign.id);
    setAutoScheduleBusyIds((current) => new Set(current).add(campaign.id));
    setMessage(""); setError("");
    try {
      const result = await setAdsterraAutoScheduleEnabledAction(shopeeAccountId, campaign.id, enabled);
      if (!result.success) { setError(result.message); return; }
      setAutoScheduleByConfig((current) => ({ ...current, [campaign.id]: result.autoScheduleEnabled }));
      setMessage(`Auto ON/OFF ${campaign.label || campaign.campaignId} ${result.autoScheduleEnabled ? "diaktifkan" : "dinonaktifkan"}.`);
    } finally {
      autoScheduleLocks.current.delete(campaign.id);
      setAutoScheduleBusyIds((current) => { const next = new Set(current); next.delete(campaign.id); return next; });
    }
  }
  ...
  return <section className="min-w-0 space-y-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
    ...
    <div className="overflow-hidden rounded-lg border border-slate-200"><div className="overflow-x-auto"><table className="w-full min-w-200 text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Campaign</th><th className="px-4 py-3">Campaign ID</th><th className="px-4 py-3">Tag_link1</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Auto ON/OFF</th><th className="px-4 py-3">Manual</th></tr></thead><tbody className="divide-y divide-slate-200">{campaigns.map((campaign) => {
      const state = statusByConfig[campaign.id]; const status = state?.campaignStatus?.status; const toggling = togglingIds.has(campaign.id); const autoScheduleBusy = autoScheduleBusyIds.has(campaign.id);
      return <tr key={campaign.id}><td className="px-4 py-3 font-medium text-slate-950"><button type="button" className="text-left hover:underline" onClick={() => setSelected(campaign)}>{campaign.label || campaign.campaignId}</button></td><td className="px-4 py-3 text-slate-600">{campaign.campaignId}</td><td className="px-4 py-3 text-slate-600">{campaign.sourceTag}</td><td className="px-4 py-3"><span className={status === "ACTIVE" ? "text-emerald-700" : status === "INACTIVE" ? "text-slate-700" : status ? "text-amber-700" : "text-slate-500"}>{status ? statusLabels[status] : state?.error ? "Tidak diketahui" : statusesBusy ? "Memuat..." : "Belum dimuat"}</span>{state?.error && <p className="mt-1 max-w-60 text-xs text-red-600">{state.error}</p>}</td><td className="px-4 py-3"><label className="inline-flex items-center gap-2 text-xs font-medium text-slate-600"><input type="checkbox" checked={autoScheduleByConfig[campaign.id] ?? campaign.autoScheduleEnabled} disabled={autoScheduleBusy} onChange={(event) => void toggleAutoSchedule(campaign, event.target.checked)} className="size-4 accent-blue-600" aria-label={`Auto ON/OFF ${campaign.label || campaign.campaignId}`} />{autoScheduleBusy ? "Menyimpan..." : (autoScheduleByConfig[campaign.id] ?? campaign.autoScheduleEnabled) ? "Aktif" : "Nonaktif"}</label></td><td className="px-4 py-3">{(status === "ACTIVE" || status === "INACTIVE" || status === "LIMITED") ? <Button type="button" size="icon" variant={status === "INACTIVE" ? "default" : "destructive"} disabled={toggling || statusesBusy} aria-label={status === "INACTIVE" ? `Aktifkan ${campaign.label || campaign.campaignId}` : `Matikan ${campaign.label || campaign.campaignId}`} title={status === "INACTIVE" ? "Aktifkan campaign" : "Matikan campaign"} onClick={() => { setConfirmationError(""); setPendingToggle({ campaign, desiredActive: status === "INACTIVE" }); }}><Power className="size-4" /></Button> : <span className="text-xs text-slate-500">{status ? "Tidak ada aksi manual" : "—"}</span>}</td></tr>;
    })}{campaigns.length === 0 && <tr><td colSpan={6} className="px-6 py-10 text-center text-slate-500">Belum ada campaign Adsterra.</td></tr>}</tbody></table></div></div>
