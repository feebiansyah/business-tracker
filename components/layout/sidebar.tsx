...
export function Sidebar({ shopeeAccounts }: { shopeeAccounts: SidebarShopeeAccount[] }) {
  ...
  return <SidebarNavigation key={pathname} pathname={pathname} shopeeAccounts={shopeeAccounts}/>;
}

function SidebarNavigation({ pathname, shopeeAccounts }: { pathname: string; shopeeAccounts: SidebarShopeeAccount[] }) {
        ...
        {showShopeeAccounts && <div className="ml-4 mt-1.5 space-y-1.5 border-l border-slate-700/60 pl-2.5">
          {shopeeAccounts.map((account) => {
            ...
            return <div key={account.id}>
              <div className="flex items-center gap-1">
                <Link href={`/shopee/${account.id}`} className={cn("min-w-0 flex-1 rounded-lg px-2.5 py-1.5 text-sm font-medium transition-colors", state.accountActive ? "bg-slate-800 text-white" : "text-slate-300 hover:bg-slate-800/70 hover:text-white")}>
