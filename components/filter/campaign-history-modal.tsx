...
export function CampaignHistoryModal({ mode, shopeeAccountId, campaignId, campaignName, onClose }: { mode: CampaignMode; shopeeAccountId: number; campaignId: number; campaignName: string; onClose: () => void }) {
  ...
  return <div role="dialog" aria-modal="true" aria-label={`Histori Harian ${campaignName}`} className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-2 sm:p-4" onClick={() => { if (modalShouldClose("overlay")) onClose(); }}>
    <div className="flex max-h-[95vh] w-[98vw] flex-col overflow-hidden rounded-xl bg-slate-50 shadow-2xl" onClick={(event) => event.stopPropagation()}>
      <div className="flex items-center justify-between gap-4 border-b border-slate-200 bg-white px-4 py-3"><div className="min-w-0"><p className="text-xs font-medium text-slate-500">Histori Harian</p><h2 className="truncate text-lg font-semibold text-slate-950">{campaignName}</h2></div><div className="flex items-center gap-2"><Link href={`/shopee/${shopeeAccountId}/${campaignModeConfig[mode].route}/${campaignId}?${historyParamsToSearch(state)}`} className="text-xs text-slate-500 hover:underline">Buka halaman penuh</Link><button type="button" aria-label="Tutup modal" className="flex size-8 items-center justify-center rounded-md text-xl text-slate-500 hover:bg-slate-100" onClick={() => { if (modalShouldClose("close-button")) onClose(); }}>×</button></div></div>
