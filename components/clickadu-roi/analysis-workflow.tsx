...
export function ClickaduAnalysisWorkflow({ shopeeAccountId, shopeeAccountName, configs, connected }: Props) {
  ...
  async function run(formData: FormData) {
    ...
    try {
      ...
      if (response.success) { lastAnalysisForm.current = cloneFormData(formData); setResult(response.analysis); setAnalysisVersion((value) => value + 1); }
