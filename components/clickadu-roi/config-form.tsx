...
export function ClickaduConfigForm({ shopeeAccountId, configs }: { shopeeAccountId: number; configs: ClickaduAnalysisConfig[] }) {
  ...
  return <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6"><div><h3 className="font-semibold text-slate-950">Konfigurasi Campaign</h3><p className="mt-1 text-sm text-slate-500">Source Tag dicocokkan ke Tag_link1 pada CSV Shopee.</p></div>
    <form action={save} className="grid gap-3 md:grid-cols-4"><input type="hidden" name="configId" value={editing?.id ?? ""}/><input name="campaignId" defaultValue={editing?.campaignId ?? ""} required placeholder="Campaign ID" className="h-9 rounded-lg border border-slate-200 px-3 text-sm"/><input name="label" defaultValue={editing?.label ?? ""} placeholder="Label (opsional)" className="h-9 rounded-lg border border-slate-200 px-3 text-sm"/><input name="sourceTag" defaultValue={editing?.sourceTag ?? ""} required placeholder="Source Tag / Tag_link1" className="h-9 rounded-lg border border-slate-200 px-3 text-sm"/><Button disabled={busy}>{editing ? "Simpan Perubahan" : "Tambah Config"}</Button></form>
