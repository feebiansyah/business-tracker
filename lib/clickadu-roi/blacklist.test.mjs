...
test("replacement succeeds only after an exact post-write verification", async () => {
  ...
  assert.deepEqual(result, { status: "UPDATED", blockedZoneCount: 2, candidateZoneIds: ["1002", "1004"] });
  assert.deepEqual(fixture.calls.map((call) => Array.isArray(call) ? call[0] : call), ["campaign", "blocked", "put", "blocked"]);
});

test("NO_CHANGE skips Clickadu PUT", async () => {
  const fixture = dependencies({ after: [] });
  fixture.deps.getBlockedZones = async () => { fixture.calls.push("blocked"); return ["1004", "1002"]; };
  const result = await replaceClickaduBlacklist("campaign-1", ["1002", "1004"], fixture.deps);
  assert.equal(result.status, "NO_CHANGE");
  assert.equal(fixture.calls.some(Array.isArray), false);
});
