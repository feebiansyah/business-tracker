...
test("Clickadu blocked zones accept the real top-level array response", async () => {
  const client = new ClickaduClient({
    token,
    fetchImpl: async () => Response.json(["1963676", "1964158", "1990704"]),
  });

  assert.deepEqual(await client.getBlockedZones("campaign-42"), [
    "1963676",
    "1964158",
    "1990704",
  ]);
