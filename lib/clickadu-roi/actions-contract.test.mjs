...
test("analysis action authenticates and accepts only config/date/rate/file truth", async () => {
  ...
  const analysisAction = source.slice(source.indexOf("export async function analyzeClickaduRoiAction"), source.indexOf("export async function replaceClickaduBlacklistAction"));
  assert.doesNotMatch(analysisAction, /formData\.get\(["']campaignId/);
  assert.doesNotMatch(analysisAction, /formData\.get\(["']sourceTag/);
  assert.doesNotMatch(analysisAction, /replaceClickaduBlacklist|updateCampaign|method:\s*["'](?:PUT|PATCH|DELETE)/);
});
