...
function validateHeaders(candidate: Candidate) {
  for (const required of REQUIRED_HEADERS) {
    ...
    if (count > 1) return { valid: false as const, error: `Header ${required} duplikat.` };
    if (count === 0) return { valid: false as const, error: `Header ${required} wajib ada.` };
  }
  return { valid: true as const };
}

export function decodeClickaduShopeeCsv(bytes: Uint8Array): ClickaduShopeeCsvRow[] {
  ...
  if (valid.length !== 1) {
    if (valid.length > 1) throw new ShopeeImportError("CSV_AMBIGUOUS_DELIMITER", "Delimiter CSV ambigu.");
  ...
  if (candidate.records.length === 0) throw new ShopeeImportError("CSV_EMPTY", "CSV tidak memiliki data.");
