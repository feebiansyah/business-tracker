...
test("configuration CRUD stays scoped to its Shopee account", async () => {
  await assert.rejects(
    prisma.$transaction(async (tx) => {
      ...
      await assert.rejects(
        saveClickaduConfig(tx, accountA.id, { campaignId: "another-campaign", label: null, sourceTag: " adu " }),
        /Source Tag Clickadu sudah digunakan campaign lain/i,
      );
      const editedOwnTag = await saveClickaduConfig(tx, accountA.id, {
        id: configA.id, campaignId: configA.campaignId, label: "Edited", sourceTag: " adu ",
      });
      assert.equal(editedOwnTag.sourceTag, "ADU");
      const sameTagOtherAccount = await saveClickaduConfig(tx, accountB.id, {
        campaignId: "other-account-campaign", label: null, sourceTag: "ADU",
      });
      assert.equal(sameTagOtherAccount.sourceTag, "ADU");
