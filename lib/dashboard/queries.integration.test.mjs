...
test("dashboard isolates Shopee accounts and aggregates, filters, sorts, and paginates in the database", async () => {
  await assert.rejects(prisma.$transaction(async (tx) => {
    ...
    const [wlA, wlA2, wlB] = await Promise.all([
      tx.metaAccount.create({ data: { name: "WL A", accountId: `dashboard-wl-a-${suffix}`, shopeeAccountId: shopeeA.id } }),
      tx.metaAccount.create({ data: { name: "WL A2", accountId: `dashboard-wl-a2-${suffix}`, shopeeAccountId: shopeeA.id } }),
    ...
    await tx.metaAccountDailySpend.createMany({ data: dates.map((date, index) => ({ metaAccountId: wlA.id, date: at(date), spend: index === 9 ? 100 : 10 })) });
    await tx.metaAccountDailySpend.create({ data: { metaAccountId: wlA2.id, date: at("2026-08-10"), spend: 25 } });
    await tx.metaAccountDailySpend.create({ data: { metaAccountId: wlB.id, date: at("2026-08-10"), spend: 999 } });
    await Promise.all([
    ...
    assert.equal(accountA.wlCount, 2);
    assert.equal(accountA.pagination.total, 26);
    assert.equal(accountA.days.length, 1);
    assert.equal(accountA.summary.budget, 215);
    assert.equal(accountA.summary.commission, 200);
    ...
    assert.equal(rangedA.days[0].budget, 125);
    assert.equal(rangedA.summary.profit, 68.75);
    throw new Rollback();
