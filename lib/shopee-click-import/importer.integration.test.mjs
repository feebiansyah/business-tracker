...
test("final click import writes matched and unmatched, ignores invalid scope rows, and rolls back", async () => {
  ...
  await assert.rejects(prisma.$transaction(async (tx) => {
    ...
    assert.equal(history.processedRowCount, 3); assert.equal(history.ignoredRowCount, 1); assert.equal(history.unmatched.length, 2); assert.deepEqual(history.unmatched.map((row) => row.tagLink2).sort(), ["MISSING", "X"]);
    throw new Rollback();
