...
test("processes META Tag Link 2 and ignores non-META or blank Tag Link 2", () => {
  const parsed = parseShopeeClickCsv(encode(csv));
  assert.equal(parsed.csvRowCount, 5);
  assert.equal(parsed.processedRowCount, 3);
  assert.equal(parsed.ignoredRowCount, 2);
...
test("preview loads scoped campaigns once and performs zero writes", async () => {
  ...
  assert.equal(preview.processedRowCount, 3);
  assert.equal(preview.ignoredRowCount, 2);
  assert.equal(preview.matchedClicks, 2);
  assert.equal(preview.unmatchedClicks, 1);
