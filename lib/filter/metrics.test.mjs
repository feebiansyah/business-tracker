...
test("Meta update payload never overwrites nullable Shopee fields", () => {
  ...
  assert.equal("completed" in update, false);
});

test("CPC Shopee is commission per Shopee click with explicit null and zero semantics", () => {
  assert.equal(calculateFinancialMetrics(999999, 100000, 10, 50).cpcShopee, 2000);
  assert.equal(calculateFinancialMetrics(999999, null, 10, 50).cpcShopee, null);
  assert.equal(calculateFinancialMetrics(999999, 100000, 10, null).cpcShopee, null);
  assert.equal(calculateFinancialMetrics(999999, 100000, 10, 0).cpcShopee, null);
  assert.equal(calculateFinancialMetrics(999999, 0, 10, 50).cpcShopee, 0);
});
