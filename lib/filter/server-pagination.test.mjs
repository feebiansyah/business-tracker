...
test("filter URL state validates defaults and allowed page sizes", () => {
  assert.deepEqual(parseFilterParams({}), { q: "", from: "", to: "", sort: "totalSpend", dir: "desc", page: 1, pageSize: 25 });
  assert.deepEqual(parseFilterParams({ q: " baby ", from: "2026-09-01", to: "2026-09-30", sort: "profit", dir: "asc", page: "3", pageSize: "50" }), { q: "baby", from: "2026-09-01", to: "2026-09-30", sort: "profit", dir: "asc", page: 3, pageSize: 50 });
...
test("filter search, sort, direction, and page-size changes reset page", () => {
  ...
  for (const change of [{ q: "two" }, { from: "2026-09-01" }, { to: "2026-09-30" }, { sort: "profit" }, { dir: "desc" }, { pageSize: 50 }]) {
...
test("filter reset clears search/range/sort/page while preserving page size", () => {
  ...
  assert.deepEqual(resetFilterParams(current), { q: "", from: "", to: "", sort: "totalSpend", dir: "desc", page: 1, pageSize: 100 });
});
