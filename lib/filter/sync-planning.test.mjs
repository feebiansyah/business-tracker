...
test("legacy metric semantics force one historical backfill from campaign start", () => {
  const result = planCampaignCoverage([{
    id: 8,
    startDate: "2026-06-01",
    historySyncedThrough: "2026-09-09",
    metricSemanticVersion: 1,
  }], "2026-09-09");
  assert.equal(result.chunks[0].since, "2026-06-01");
  assert.equal(result.chunks.at(-1).until, "2026-09-09");
  assert.deepEqual(result.semanticBackfillCampaignIds, [8]);
});

test("current semantics retain initial history and routine D-1 correction", () => {
  const initial = planCampaignCoverage([{ id: 9, startDate: "2026-09-01", historySyncedThrough: null, metricSemanticVersion: 2 }], "2026-09-09");
  assert.equal(initial.chunks[0].since, "2026-09-01");
  assert.deepEqual(initial.semanticBackfillCampaignIds, []);
  const routine = planCampaignCoverage([{ id: 9, startDate: "2026-09-01", historySyncedThrough: "2026-09-09", metricSemanticVersion: 2 }], "2026-09-10");
  assert.deepEqual(routine.chunks, [{ since: "2026-09-09", until: "2026-09-10", campaignIds: [9] }]);
});

test("legacy campaign without a usable start is not marked backfilled", () => {
  const result = planCampaignCoverage([{ id: 10, startDate: null, historySyncedThrough: "2026-09-09", metricSemanticVersion: 1 }], "2026-09-09");
  assert.deepEqual(result.chunks, []);
  assert.deepEqual(result.semanticBackfillCampaignIds, []);
  assert.deepEqual(result.missingStartCampaignIds, [10]);
});
