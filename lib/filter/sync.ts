...
import { resolveEffectiveDailyBudget, isFilterCampaign } from "@/lib/filter/rules";
...
export async function syncFilter(shopeeAccountId: number): Promise<FilterSyncSummary> {
  ...
  for (const [wlIndex, wl] of account.metaAccounts.entries()) {
    try {
      ...
      const filterCampaigns = storedCampaigns.filter((campaign) => isFilterCampaign({
        status: campaign.metaStatus,
        effectiveDailyBudget: campaign.effectiveDailyBudget?.toNumber() ?? null,
      }));
      const coverage = planCampaignCoverage(filterCampaigns.map((campaign) => ({
        id: campaign.id,
      ...
      const byId = new Map(filterCampaigns.map((campaign) => [campaign.id, campaign]));
