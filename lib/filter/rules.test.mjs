...
import { isFilterCampaign, isHistorySyncCampaign, resolveEffectiveDailyBudget } from "./rules.ts";
...
test("Meta history sync includes all four resolved campaign modes", () => {
  assert.equal(isHistorySyncCampaign({ status: "ACTIVE", effectiveDailyBudget: 199999 }), true);
  assert.equal(isHistorySyncCampaign({ status: "ACTIVE", effectiveDailyBudget: 200000 }), true);
  assert.equal(isHistorySyncCampaign({ status: "PAUSED", effectiveDailyBudget: 199999 }), true);
  assert.equal(isHistorySyncCampaign({ status: "ARCHIVED", effectiveDailyBudget: 200000 }), true);
});

test("Meta history sync excludes only unresolved campaign budgets", () => {
  assert.equal(isHistorySyncCampaign({ status: "ACTIVE", effectiveDailyBudget: null }), false);
  assert.equal(isHistorySyncCampaign({ status: "PAUSED", effectiveDailyBudget: null }), false);
});
