...
test("sync requests account-level daily spend in bounded ranges", async () => {
  const source = await readFile(new URL("./sync.ts", import.meta.url), "utf8");
  assert.match(source, /getAccountSpendHistoryStart/);
  assert.match(source, /getAccountDailySpend\(wl\.accountId, range\)/);
  assert.match(source, /persistMetaAccountDailySpend/);
  assert.doesNotMatch(source, /next\.setUTCDate\(next\.getUTCDate\(\) \+ 1\)/);
});

test("routine sync has no semantic-version backfill branch or completion marker", async () => {
  const source = await readFile(new URL("./sync.ts", import.meta.url), "utf8");
  assert.doesNotMatch(source, /metaMetricSemanticVersion/);
  assert.doesNotMatch(source, /markCampaignMetricSemanticBackfillComplete/);
});
