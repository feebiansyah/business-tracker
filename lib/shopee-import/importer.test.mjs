...
test("locks before loading Meta and Clickadu data and persistence",async()=>{const d=deps();await importShopeeCommissions({shopeeAccountId:2,originalFilename:"x.csv",bytes,confirmation:await confirmation()},d);assert.deepEqual(d.events,["tx","lock","load","load-clickadu","persist"])});
