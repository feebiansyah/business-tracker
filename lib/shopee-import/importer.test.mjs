...
test("locks before loading Meta, Clickadu, and Adsterra data once before persistence",async()=>{const d=deps();await importShopeeCommissions({shopeeAccountId:2,originalFilename:"x.csv",bytes,confirmation:await confirmation()},d);assert.deepEqual(d.events,["tx","lock","load","load-clickadu","load-adsterra","persist"])});
