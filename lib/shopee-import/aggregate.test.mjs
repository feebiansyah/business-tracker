...
test("groups by date and normalized tag with exact sums and row counts", () => {
  ...
  assert.equal(result.totalCommission.toFixed(2), "1.30");
  assert.deepEqual(
...
test("aggregates actual DUNIACUPITA decimal commissions exactly", () => {
  const records = decodeAndParseCsv(
    encode(`Waktu Pemesanan,Tag_link2,Komisi Bersih Affiliate (Rp)
2026-08-04 09:00:00,DUNIACUPITA,6052
2026-08-04 10:00:00,DUNIACUPITA,0
2026-08-04 11:00:00,DUNIACUPITA,1316.7
2026-08-04 12:00:00,DUNIACUPITA,980.475`),
  );

  const result = aggregateCommissionRows(records);

  assert.equal(result.aggregates.length, 1);
  assert.equal(result.aggregates[0].normalizedTagLink2, "DUNIACUPITA");
  assert.equal(result.aggregates[0].commission.toFixed(5), "8349.17500");
  assert.equal(result.aggregates[0].rowCount, 4);
});
