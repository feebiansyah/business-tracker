...
test("history verifies config ownership and paginates newest rows server-side", async () => {
  ...
  const db = {
    ...
    clickaduCampaignDailyMetric: {
      ...
      findMany: async (args) => { calls.push(["rows", args]); return [{ id: 1, date: new Date("2026-09-19T00:00:00.000Z"), spendUsd: null, dailyBudget: null, commissionIdr: { toString: () => "125000.00000" } }]; },
    },
  };
  ...
  assert.equal(result?.rows[0].commissionIdr, "125000.00000");
