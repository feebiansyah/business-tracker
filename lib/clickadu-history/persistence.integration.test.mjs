...
test("daily metric upsert is idempotent and preserves other dates", async () => {
  ...
  await assert.rejects(prisma.$transaction(async (tx) => {
    const config = await tx.clickaduCampaignConfig.create({
      data: { shopeeAccountId: account.id, campaignId: `daily-${randomUUID()}`, sourceTag: `DAILY-${randomUUID()}` },
    });
    await tx.clickaduCampaignDailyMetric.create({ data: {
      clickaduCampaignConfigId: config.id,
      date: new Date("2026-09-18T00:00:00.000Z"),
      commissionIdr: "125000.00000",
      spendUsd: "20",
      dailyBudget: "100",
    } });
    await upsertClickaduDailyMetric(tx, { clickaduCampaignConfigId: config.id, date: "2026-09-18", spendUsd: null, dailyBudget: null });
    const cleared = await tx.clickaduCampaignDailyMetric.findUniqueOrThrow({ where: { clickaduCampaignConfigId_date: { clickaduCampaignConfigId: config.id, date: new Date("2026-09-18T00:00:00.000Z") } } });
    assert.equal(cleared.spendUsd, null);
    assert.equal(cleared.dailyBudget?.toString(), "100");
    ...
    assert.equal(rows[0].commissionIdr?.toString(), "125000");
