...
import { PrismaClient, TrafficProvider } from "../lib/generated/prisma/client.ts";
...
export function normalizePlacementIds(values) {
  if (!Array.isArray(values)) throw new Error("Response blacklist Adsterra tidak valid.");
...
export function buildReplacePayload(placementIds) {
  return {
    ...
    placement_ids: normalizePlacementIds(placementIds),
  };
}

export function assertTestGuard(mode, confirmation) {
  if (mode === "test" && confirmation !== CONFIRMATION) {
    throw new Error(`Mode test memerlukan konfirmasi persis: ${CONFIRMATION}`);
  }
}
...
async function safeFetch(url, init) {
  ...
  if (!response.ok) throw new Error(`Adsterra API gagal dengan HTTP ${response.status}.`);
  return response;
}
...
async function loadApiKey(shopeeAccountId) {
  ...
  if (!credential) throw new Error("Credential ADSTERRA belum dikonfigurasi untuk Shopee Account ini.");
  try {
    return { account, apiKey: decryptTrafficSecret(credential.encryptedSecret) };
  } catch {
...
async function inspect(apiKey, account) {
  ...
  console.log(`Placement IDs: ${placements.length ? placements.join(", ") : "(kosong)"}`);
}

async function runTest(apiKey, account) {
  const original = await requestBlacklist(apiKey);
  if (original.length < 2) throw new Error("Blacklist awal harus memiliki minimal 2 placement untuk safe probe.");
  ...
  try {
    ...
    if (!samePlacementSet(after, testPlacements)) throw new Error("Verifikasi blacklist test tidak cocok.");
    testPassed = true;
  ...
  if (!testPassed || !restorePassed) throw new Error("Safe write probe Adsterra tidak selesai dengan aman.");
}

async function main() {
  const [mode, accountIdValue, confirmation] = process.argv.slice(2);
  if (!new Set(["inspect", "test"]).has(mode)) throw new Error("Mode harus inspect atau test.");
  const shopeeAccountId = Number(accountIdValue);
  if (!Number.isSafeInteger(shopeeAccountId) || shopeeAccountId <= 0) throw new Error("Shopee Account ID tidak valid.");
  assertTestGuard(mode, confirmation);

  const { account, apiKey } = await loadApiKey(shopeeAccountId);
  if (mode === "inspect") await inspect(apiKey, account);
  else await runTest(apiKey, account);
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main()
    .catch((error) => {
      console.error(error instanceof Error ? error.message : "Safe write probe Adsterra gagal.");
      process.exitCode = 1;
    })
