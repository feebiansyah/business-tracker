...
export default async function AllWlPage({ searchParams }: { searchParams: Promise<{ connection?: string | string[] }> }) {
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-medium text-slate-500">Master data</p><h2 className="text-2xl font-semibold tracking-tight text-slate-950">Semua WL</h2></div><div className="flex flex-wrap items-start gap-3"><MetaSyncButton /><MetaBusinessMappingButton /></div></div>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><table className="w-full min-w-225 text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-6 py-3 font-medium">Nama WL</th><th className="px-6 py-3 font-medium">Account ID</th><th className="px-6 py-3 font-medium">Business Manager</th><th className="px-6 py-3 font-medium">Akun Shopee</th><th className="px-6 py-3 font-medium">Status WL</th><th className="px-6 py-3 font-medium">Status koneksi</th><th className="px-6 py-3"><span className="sr-only">Aksi</span></th></tr></thead><tbody className="divide-y divide-slate-200">{metaAccounts.map((account) => <tr key={account.id}><td className="px-6 py-4 font-medium text-slate-900">{account.name}</td><td className="px-6 py-4 text-slate-600">{account.accountId}</td><td className="px-6 py-4 text-slate-600">{account.businessManager?.name ?? "Tanpa BM"}</td><td className="px-6 py-4 text-slate-600">{account.shopeeAccount?.name ?? "—"}</td><td className="px-6 py-4"><StatusBadge status={account.status} /></td><td className="px-6 py-4"><ConnectionStatusBadge connected={account.shopeeAccountId !== null} /></td><td className="px-6 py-4 text-right"><Link href={`/wl/${account.id}`} className="font-medium text-slate-900 hover:underline">{account.shopeeAccountId === null ? "Hubungkan" : "Ganti akun"}</Link></td></tr>)}{metaAccounts.length === 0 && <tr><td colSpan={7} className="px-6 py-8 text-center text-slate-500">Tidak ada WL untuk filter ini.</td></tr>}</tbody></table></div>
    </section>
  );
}
