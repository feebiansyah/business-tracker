...
export async function previewShopeeCommissionAction(shopeeAccountId:number,formData:FormData):Promise<PreviewActionResult>{await requireUser();try{const upload=await readCsvUpload(shopeeAccountId,formData);return {success:true,preview:await buildShopeeCommissionPreview({shopeeAccountId,...upload},realPreviewDeps)}}catch(error){console.error("[Shopee Commission Preview]",error);return {success:false,message:publicImportMessage(error)}}}

export async function importShopeeCommissionAction(shopeeAccountId:number,confirmation:PreviewConfirmation,formData:FormData){await requireUser();try{validatePreviewConfirmation(confirmation);const upload=await readCsvUpload(shopeeAccountId,formData);const receipt=await importShopeeCommissions({...upload,shopeeAccountId,confirmation},{withTransaction:work=>prisma.$transaction(work,{isolationLevel:Prisma.TransactionIsolationLevel.ReadCommitted,timeout:30000}),lockAccount:lockShopeeAccount,loadCampaigns:loadShopeeCampaignCandidates,loadClickaduConfigs:(tx,id)=>tx.clickaduCampaignConfig.findMany({where:{shopeeAccountId:id},select:{id:true,sourceTag:true}}),loadAdsterraConfigs:(tx,id)=>tx.adsterraCampaignConfig.findMany({where:{shopeeAccountId:id},select:{id:true,sourceTag:true}}),persist:persistCommissionImportInTransaction});revalidatePath(`/shopee/${shopeeAccountId}/import`);revalidatePath(`/shopee/${shopeeAccountId}/clickadu-roi`);revalidatePath(`/shopee/${shopeeAccountId}/adsterra-roi`);return {success:true as const,receipt}}catch(error){console.error("[Shopee Commission Import]",error);return {success:false as const,message:publicImportMessage(error)}}}

export async function previewShopeeClickAction(shopeeAccountId:number,formData:FormData){await requireUser();try{const upload=await readCsvUpload(shopeeAccountId,formData);return {success:true as const,preview:await buildShopeeClickPreview({shopeeAccountId,...upload},clickPreviewDeps)}}catch(error){console.error("[Shopee Click Preview]",error);return {success:false as const,message:publicImportMessage(error)}}}
export async function importShopeeClickAction(shopeeAccountId:number,confirmation:PreviewConfirmation,formData:FormData){await requireUser();try{validatePreviewConfirmation(confirmation);const upload=await readCsvUpload(shopeeAccountId,formData);const receipt=await importShopeeClicks({...upload,shopeeAccountId,confirmation},{withTransaction:work=>prisma.$transaction(work,{isolationLevel:Prisma.TransactionIsolationLevel.ReadCommitted,timeout:30000}),lockAccount:lockShopeeAccount,loadCampaigns:loadShopeeCampaignCandidates,persist:persistClickImportInTransaction});revalidatePath(`/shopee/${shopeeAccountId}/import`);return {success:true as const,receipt}}catch(error){console.error("[Shopee Click Import]",error);return {success:false as const,message:publicImportMessage(error)}}}
